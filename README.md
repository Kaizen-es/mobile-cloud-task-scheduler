...
Generates minimal-delay scheduling to minimize application completion time (T^total):
...
- Evaluates migration candidates (local core ↔ cloud, local core ↔ local core)
- Uses linear-time O(N) rescheduling algorithm to maintain precedence constraints
- Ensures final schedule satisfies T^total ≤ T^max constraint
...
Default system parameters (matching paper):
- **Local cores**: 3 heterogeneous cores with power consumption P1=0.9W, P2=0.6W, P3=0.3W
- **Cloud parameters**: T_send=2, T_cloud=1, T_receive=1
- **Wireless power**: P_s=1.3W
